...
> 複数の対象を続けて実行する場合は、**1つずつ**行ってください。
> 前の Unity プロセスがプロジェクトのロックを解放しきる前に次を起動すると、
> 起動直後に終了して失敗します。

### ブラウザ版（WebGL）

`file://` では動きません。ブラウザがローカルファイルへの `fetch` を拒否するためです。
HTTP で配信する必要があります。同梱のスクリプトが使えます。

```powershell
tools\Serve-WebGL.ps1 -Path "$env:USERPROFILE\Desktop\三か月の春_WebGL"
```

.NET の `HttpListener` を使うので Node や Python は不要です。
起動すると LAN 内の他端末から開ける URL も表示されるので、
**スマートフォンの実機確認にも使えます**（同じ Wi-Fi に接続している必要があります）。

WebGL 固有の対応:

- `Assets/WebGLTemplates/Responsive/` が専用テンプレートです。
  Unity 既定のテンプレートはデスクトップでキャンバスを 1920x1080 に固定するため、
  それより小さいウィンドウでは画面が見切れます
- `autoSyncPersistentDataPath` を有効にしています。既定は無効で、
  そのままだとセーブと既読の記録がタブを閉じた時点で消えます
- 「終了」ボタンは非表示にしています。ブラウザでは `Application.Quit()` が
  何も起こさないためです
- 圧縮は無効にしています。Brotli / Gzip は `Content-Encoding` を返せるサーバが要るので、
  静的ホスティングでも確実に動くほうを選んでいます（そのぶん容量は大きめです）
